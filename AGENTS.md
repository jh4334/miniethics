...
- `src/data/curriculum.ts`, `src/data/lessons/` — 12차시 주제·스토리 대사·퀴즈 데이터
- `src/assets-manifest.ts` — 캐릭터·배경 이미지 로더 (WebP → PNG → 캐릭터 SVG 폴백)
- `public/assets/` — 완성된 캐릭터 14종·배경 14종과 캐릭터 SVG 폴백
- `docs/` — 커리큘럼 설계, 에셋 가이드

## 자주 하는 작업

- **캐릭터 에셋 교체**: `docs/ASSET_TASK.md`의 지시를 그대로 따를 것
- **새 미니게임 추가**: `src/games/gameXX-이름.ts` 생성 → `registry.ts` 등록 → 1~3차시는 `curriculum.ts`, 4~12차시는 `data/lessons/lessonXX.ts`에 스토리/퀴즈 작성

## 규칙

- 확인 명령: `npm ci && npm run check:ci && npm audit --audit-level=moderate`. 하나라도 깨지면 작업 완료가 아님
