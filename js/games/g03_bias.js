...
import { buildStage, countdown, gameTimer, floatScore, toast, starsFromRatio, randInt, pick } from './engine.js';
    ...
    function spawn() {
      ...
      el.addEventListener('pointerdown', (ev) => {
        ...
        if (spread >= 3) {
          ...
          robot.textContent = '🥴';
          robotSay.textContent = `${TYPES.find((t) => t.key === type.key).name} 데이터만 너무 많아요!`;
        } else {
          score += 5;
          sfx.pop();
    ...
    const stopCd = countdown(host, {
      ...
      timer = gameTimer(TIME, (t) => ui.setTime(t), () => {
        ...
        ctx.finish({
          ...
          msg: balanced
            ? '데이터를 골고루 모았어요! 다양한 데이터가 공평한 AI를 만들어요 🌈'
            : '데이터가 한쪽으로 치우쳤네요. 치우친 데이터는 편향된 AI를 만들어요!',
        });
      });
    });
